<template>
  ...
  export default {
    ...
    const updateMarkers = () => {
      ...
      const filteredCampaigns = this.filteredCampaigns.filter((campaign) =>
        campaign.location.toLowerCase().includes(this.searchQuery.toLowerCase())
      );
    ...
    watch(
      () => [this.filteredCampaigns, this.searchQuery],
      () => {
        updateMarkers();
      },
      { deep: true }
